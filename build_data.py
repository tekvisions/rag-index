#!/usr/bin/env python3
"""The RAG Index — recompute the living index of retrieval-augmented-generation tooling from
live GitHub signals, and write data.json + SEO (sitemap, rss, robots, llms.txt).

Scope = the RAG stack: RAG frameworks (ragflow, haystack, graphrag), vector databases (milvus,
qdrant, weaviate, lancedb, chroma), embeddings/reranking, and document parsing for ingestion.
Excludes broad agent platforms (langchain, dify) and repos that belong to sibling indexes
(local-LLM apps, etc.). Gathered, deduped, FILTERED (precision over recall), categorized, scored.
...
SITE_URL = "https://rag-index.vercel.app"   # fixed to the real alias after first deploy
SITE_NAME = "The RAG Index"

QUERIES = [
    "topic:rag stars:>40",
    "topic:vector-database stars:>30",
    "topic:retrieval-augmented-generation stars:>20",
    "rag framework in:name,description stars:>50",
    "retrieval augmented generation in:name,description stars:>40",
    "vector database in:name,description stars:>80",
    "embeddings search in:name,description stars:>60",
    "reranking retrieval in:name,description stars:>20",
]
...
HEADERS = {"Accept": "application/vnd.github+json", "User-Agent": "rag-index"}
if token():
    HEADERS["Authorization"] = f"Bearer {token()}"

_RAG_TOPICS = {"rag", "retrieval-augmented-generation", "vector-database", "vector-search",
               "embeddings", "semantic-search", "reranking", "graphrag", "vector-store"}
_RAG_PHRASES = re.compile(
    r"\b(retrieval[- ]augmented|rag (framework|pipeline|system|engine)|vector (database|db|store|search)"
    r"|embedding(s)? (search|database|index)|semantic search|reranker|reranking|graph ?rag"
    r"|knowledge base|document (q&a|qa|retrieval))\b", re.I)
# Broad platforms / sibling-index repos / non-RAG repos that match but aren't RAG-stack tools.
_DENY = {"langchain-ai/langchain", "langgenius/dify", "open-webui/open-webui",
         "mintplex-labs/anything-llm", "n8n-io/n8n", "logspace-ai/langflow", "flowiseai/flowise",
         "safishamsi/graphify", "jeecgboot/jeecgboot", "simstudioai/sim",
         "datawhalechina/hello-agents", "thedotmack/claude-mem", "mem0ai/mem0"}
_ANTI = re.compile(
    r"\b(agent platform|workflow automation|no-?code|low-?code|chatbot builder|llm app platform"
    r"|coding assistant|agent memory|memory layer|tutorial|sql injection|kubernetes operator)\b", re.I)
...
def is_rag(r: dict) -> bool:
    ...
    if topics & _RAG_TOPICS:
        return True
    return bool(_RAG_PHRASES.search(f"{r.get('name','')} {desc}"))


def categorize(r: dict) -> str:
    topics = {t.lower() for t in (r.get("topics") or [])}
    blob = f"{(r.get('name') or '').lower()} {(r.get('description') or '').lower()} {' '.join(topics)}"
    if re.search(r"graph ?rag|knowledge graph", blob):
        return "Graph RAG"
    if re.search(r"vector (database|db|store)|milvus|qdrant|weaviate|chroma|lancedb|pgvector", blob):
        return "Vector Databases"
    if re.search(r"rerank|reranker|cross-?encoder", blob):
        return "Reranking"
    if re.search(r"embedding|encoder|sentence-?transformer", blob):
        return "Embeddings"
    if re.search(r"pars|ingest|chunk|loader|extract|ocr|document", blob):
        return "Ingestion & Parsing"
    if re.search(r"semantic search|search engine|retriev", blob):
...
def build_items() -> list[dict]:
    ...
    for q in QUERIES:
        for r in search(q):
            ...
            if full and full not in seen and is_rag(r):
                seen[full] = r
        time.sleep(0.7)
...
def write_seo(data: dict) -> None:
    ...
    open(os.path.join(HERE, "rss.xml"), "w", encoding="utf-8").write(
        ...
        "    <description>The living index of retrieval-augmented-generation tooling — frameworks, vector DBs, embeddings.</description>\n"
        + "\n".join(rss_items) + "\n  </channel>\n</rss>\n")

    lines = [f"# {SITE_NAME}", "",
             "> The living index of retrieval-augmented-generation tooling — RAG frameworks,",
             "> vector databases, embeddings & reranking — ranked daily by GitHub momentum.", "",
             f"Updated: {data['generated_at']}", f"Tools indexed: {data['count']}", "",
             "## Top RAG tools by momentum", ""]
    for it in items[:40]:
...
def main() -> int:
    ...
    items = build_items()
    if not items:
        print("ERROR: no RAG tools found — refusing to write empty data.json", file=sys.stderr)
        return 1
    data = write_json(items)
    write_seo(data)
    print(f"wrote data.json: {len(items)} RAG tools across {len(data['categories'])} categories")
